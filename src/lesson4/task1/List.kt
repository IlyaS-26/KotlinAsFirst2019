...
fun abs(v: List<Double>): Double {
    val result = v.fold(0.0) { previousResult, element ->
...
fun convertToString(n: Int, base: Int): String {
    ...
    for (i in convert.indices) {
        if (convert[i] in 0..9) list.add(convert[i].toString())
        else list.add(('a' + convert[i] - 10).toString())
    }
...
fun decimal(digits: List<Int>, base: Int): Int {
    ...
    for (element in digits) {
        size--
        number += (element * powInt(base, size))
    }
    return number
}
...
fun russian(n: Int): String {
    ...
    varN = n / 100000
    result.add(hundreds(varN))
    if ((n % 100000 / 1000) in 11..19) {
        varN = n % 100000 / 1000
        result.add(thousands(varN))
    } else {
        varN = n % 100000 / 10000
        result.add(dozens(varN))
        if (i > 3) {
            varN = n % 10000 / 1000
            result.add(thousands(varN))
        }
